...
def take_images(positions, via_pos, controller, vel, DC, intr):
    ...
    images = []
    depths = []
    data = {"joints": [], "cart_pose": []}
    for i, joints in enumerate(positions):
        print(
            "currentjoints = {}, joints = {}, diff = {}".format(
                controller.get_joints(), joints, joints - controller.get_joints()
            )
        )
...
def get_camera_poses(DC, name):
    ...
    path = os.path.join(
        "/home/kochpaul/projects/masterthesis/robot_controller/robot_path", name
    )
    ...
    if controller.is_home():
        ...
        # get images
        images, depths, meta2 = take_images(
            data["joints"], data["via_points"], controller, vel, DC, intr
        )
